...
router.get('/information', authenticate, (req, res) => {
  let sql =
    'SELECT first_name, last_name, phone_number,company_name FROM customers WHERE id=?';
...
router.patch('/edit', authenticate, (req, res) => {
  let sql =
    'UPDATE customers SET first_name=?, last_name=?, phone_number=?,company_name=? WHERE id =?;';
  connection.query(
    sql,
    [
      req.body.first_name,
...
router.post('/forgotpassword', (req, res) => {
  ...
  connection.query(sql, [req.body.email, req.body.username], (err, result) => {
    if (err) {
...
router.post(
  '/resetpassword',
  restPasswordTokenAuth,
  hashPassword,
  (req, res) => {
    let sql = 'UPDATE customers SET password=? WHERE id =?';
    connection.query(sql, [req.body.password, req.body.id], (err, result) => {
      if (err) {
